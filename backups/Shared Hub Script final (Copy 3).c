...
function updateStats(doc, config) {
  try {
    ...
    // We'll always generate stats with the primary sand timer for consistency
    ...
    // --- Anywhere Timer Stats Logic (Updated for multiple emojis) ---
    const finalParas = body.getParagraphs();
    const ANYWHERE_MARKERS = ['⏳', '⏳️', '⌛️']; // Array of all valid markers
    const PRIMARY_ANYWHERE_MARKER = '⏳';       // The one we revert back to

    if (config.statsAnywhere === true) {
      for (let i = 0; i < finalParas.length; i++) {
        const paraText = finalParas[i].getText().trim();
        // Check if the paragraph is one of our markers
        if (ANYWHERE_MARKERS.includes(paraText)) {
          finalParas[i].setText(sandTimerStatsText);
        }
      }
    } else if (config.statsAnywhere === false) {
      for (let i = 0; i < finalParas.length; i++) {
        const text = finalParas[i].getText();
        // Check if it's a stats block generated by this feature
