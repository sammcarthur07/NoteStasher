...
function handleAppend(token, docId, e, payload) {
  ...
  // --- THE FIX ---
  // Insert the separator and the content as two separate paragraphs.
  // This prevents the stats logic from ever touching the separator.
  body.insertParagraph(insertAt, "—");
  body.insertParagraph(insertAt + 1, content); 
