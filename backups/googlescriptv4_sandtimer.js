// Shared Hub Script for ReplyNote App (Sand Timer Version)

function doGet(e) { return handleRequest(e); }
function doPost(e) { return handleRequest(e); }

// ==================== TEST FUNCTION ====================
function testSandTimer() {
  ...
  body.appendParagraph('⏳');  // Sand timer marker
  body.appendParagraph('Some content');
  body.appendParagraph('⏳');  // Another sand timer marker
  
  const configKey = 'config_' + TEST_TOKEN + '_' + TEST_DOC_ID;
  
  // Test enabling statsAnywhere
  Logger.log('=== Enabling statsAnywhere with sand timer ===');
  const config = { statsTop: false, statsBottom: true, statsAnywhere: true, timezone: 'UTC' };
  ...
  for (let i = 0; i < paras.length; i++) {
    Logger.log('Paragraph ' + i + ': "' + paras[i].getText().substring(0, 50) + '..."');
  }
}
...
// ==================== CORE STATS LOGIC (WITH SAND TIMER) ====================
...
function updateStats(doc, config) {
  try {
    ...
    // --- NEW: Every ⏳ Stats Logic (uses sand timer emoji) ---
