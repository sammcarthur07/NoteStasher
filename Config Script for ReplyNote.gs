...
function updateStats(doc, config) {
  try {
    ...
    // Trim blank paragraphs from the top so stats sit cleanly.
    ...
    const bigChange = Math.abs(cleanText.length - lastStoredContent.length) > 5;

    const nowMs = Date.now();

    if (bigChange || lastChangeTimeMs === null) {
