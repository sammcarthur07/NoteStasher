// Enhanced Config Script for ReplyNote / Note Stasher
// Deploy with:
//   - Execute as: Me (script owner)
//   - Who has access: Anyone
// Provides remote configuration, quota guardrails, and counters.
...
function handleRequest(e) {
  ...
  try {
    ...
    console.error('Config script error', err);
...
function setConfig(e) {
  ...
  if (e.postData && e.postData.contents) {
    try {
      ...
      console.warn('Failed to parse JSON body; using params only');
    }
  }
  ...
  if (merged.resetDailyCounters === 'true' || merged.resetDailyCounters === true) {
...
function runQuotaGuardrail(manual) {
  ...
  if (manual) {
    return {
      freeQuotaSuspended: freeSuspended,
      globalSyncSuspended: globalSuspended,
      remainingQuota: remaining,
      consecutiveSyncErrors: consecutiveErrors
    };
  }
}

function ensureDefaults(props) {
  if (!props.getProperty(PROPERTY_KEYS.HUB_URL)) props.setProperty(PROPERTY_KEYS.HUB_URL, CONFIG_DEFAULTS.HUB_URL);
  if (!props.getProperty(PROPERTY_KEYS.MASTER_PASSWORD)) props.setProperty(PROPERTY_KEYS.MASTER_PASSWORD, CONFIG_DEFAULTS.MASTER_PASSWORD);
  if (!props.getProperty(PROPERTY_KEYS.NEXT_CONFIG_URL)) props.setProperty(PROPERTY_KEYS.NEXT_CONFIG_URL, CONFIG_DEFAULTS.NEXT_CONFIG_URL);
  if (!props.getProperty(PROPERTY_KEYS.QUOTA_DAILY_LIMIT)) props.setProperty(PROPERTY_KEYS.QUOTA_DAILY_LIMIT, String(CONFIG_DEFAULTS.QUOTA_DAILY_LIMIT));
  if (!props.getProperty(PROPERTY_KEYS.FREE_DAILY_QUOTA)) props.setProperty(PROPERTY_KEYS.FREE_DAILY_QUOTA, String(CONFIG_DEFAULTS.FREE_DAILY_QUOTA));
  if (!props.getProperty(PROPERTY_KEYS.BULK_CAP_BYTES)) props.setProperty(PROPERTY_KEYS.BULK_CAP_BYTES, String(CONFIG_DEFAULTS.BULK_CAP_BYTES));
  if (!props.getProperty(PROPERTY_KEYS.FREE_QUOTA_SUSPENDED)) props.setProperty(PROPERTY_KEYS.FREE_QUOTA_SUSPENDED, String(CONFIG_DEFAULTS.FREE_QUOTA_SUSPENDED));
  if (!props.getProperty(PROPERTY_KEYS.GLOBAL_SYNC_SUSPENDED)) props.setProperty(PROPERTY_KEYS.GLOBAL_SYNC_SUSPENDED, String(CONFIG_DEFAULTS.GLOBAL_SYNC_SUSPENDED));
  if (!props.getProperty(PROPERTY_KEYS.TOTAL_CLOUD_SENDS)) props.setProperty(PROPERTY_KEYS.TOTAL_CLOUD_SENDS, '0');
  if (!props.getProperty(PROPERTY_KEYS.FREE_SENDS_TODAY)) props.setProperty(PROPERTY_KEYS.FREE_SENDS_TODAY, '0');
  if (!props.getProperty(PROPERTY_KEYS.PAID_SENDS_TODAY)) props.setProperty(PROPERTY_KEYS.PAID_SENDS_TODAY, '0');
  if (!props.getProperty(PROPERTY_KEYS.CONSECUTIVE_ERRORS)) props.setProperty(PROPERTY_KEYS.CONSECUTIVE_ERRORS, '0');
}
...
function recordSyncError(props) {
  const count = parseInteger(props.getProperty(PROPERTY_KEYS.CONSECUTIVE_ERRORS), 0) + 1;
  props.setProperty(PROPERTY_KEYS.CONSECUTIVE_ERRORS, String(count));
}
...
function writeIntegerPropertyIfPresent(props, key, value, min) {
  ...
  if (isNaN(parsed)) throw new Error('Invalid numeric value for ' + key);
  if (min !== undefined && parsed < min) throw new Error('Value for ' + key + ' must be >= ' + min);
