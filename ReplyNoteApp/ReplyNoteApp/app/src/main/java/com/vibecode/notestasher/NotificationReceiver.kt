...
class NotificationReceiver : BroadcastReceiver() {
    ...
    companion object {
        const val ACTION_DISABLE_ALL = "com.vibecode.notestasher.ACTION_DISABLE_ALL"
        const val ACTION_NOTIFICATIONS_UPDATED = "com.vibecode.notestasher.ACTION_NOTIFICATIONS_UPDATED"
    }
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        scope.launch {
            ...
            val docDisplayName = doc.alias.ifBlank { doc.title }.ifBlank { doc.docId }

            if (success) {
                SharedPreferencesHelper.saveDocLastMessage(appContext, docId, replyText)
                SharedPreferencesHelper.saveMessageToHistory(appContext, replyText, docId, docDisplayName)
