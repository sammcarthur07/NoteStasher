...
object SharedPreferencesHelper {
    private const val PREFS_NAME = "NoteStasherPrefs"
    ...
    fun saveMessageToHistory(
        ...
        val resolvedDocId = docId ?: loadSelectedDocId(context)
        val resolvedDocName = docName ?: loadDocDisplayName(context, resolvedDocId)
        val newItem = MessageHistoryItem(
            text = message,
            timestamp = timestamp,
    ...
    fun saveRichMessageToHistory(
        context: Context,
        html: String,
        plainPreview: String = "",
        attachments: List<HistoryAttachment> = emptyList(),
        docId: String? = null,
        docName: String? = null
    ) {
    ...
    fun saveSendingRichHistoryItem(
        context: Context,
        sessionId: String,
        htmlPreview: String,
        plainPreview: String = "",
    ...
    fun loadDocDisplayName(context: Context, docId: String?): String? {
        if (docId.isNullOrBlank()) return null
        val doc = loadDocEntries(context).firstOrNull { it.docId == docId }
        val alias = doc?.alias?.ifBlank { doc.title }
        return alias?.takeIf { it.isNotBlank() } ?: docId
    }
